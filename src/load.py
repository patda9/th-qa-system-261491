...
for i in range(n_samples):
    ...
    while(answer_doc_id[randomed_question] > 282972): # limited preprocessed corpus
        randomed_question = np.random.randint(questions.__len__())
    sample_question_ans.append((randomed_question, answer_doc_id[randomed_question]))
sample_question_ans = sorted(sample_question_ans)

# print(sample_question_ans)
# print(sample_questions.__len__())

# for q in answer_details['data']:
#     break
#     print(q)

# with open(path + str(6865) + '.json', 'r') as f:
#     test = json.load(f)

# test = prep.remove_xml(test)
# test = prep.remove_stop_words(test)
